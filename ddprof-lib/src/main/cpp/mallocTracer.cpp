...
static void detectNestedMalloc() {
    ...
    // _orig_malloc != NULL is guaranteed by the early-return guard above.
...
void MallocTracer::patchLibraries() {
    ...
    while (_patched_libs < native_lib_count) {
        ...
        if (_nested_malloc) {
            // Use dummy hooks to prevent double-accounting. Dummy frames are introduced
            // to preserve the frame link to the original caller.
            if (_orig_calloc) cc->patchImport(im_calloc, (void*)calloc_hook_dummy);
            if (_orig_posix_memalign) cc->patchImport(im_posix_memalign, (void*)posix_memalign_hook_dummy);
        } else {
            if (_orig_calloc) cc->patchImport(im_calloc, (void*)calloc_hook);
            if (_orig_posix_memalign) cc->patchImport(im_posix_memalign, (void*)posix_memalign_hook);
        }
    }
}
