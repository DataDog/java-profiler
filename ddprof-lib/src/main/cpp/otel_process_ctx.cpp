// Taken from https://raw.githubusercontent.com/DataDog/fullhost-code-hotspots-wip/refs/heads/ivoanjo/context-sharing-reference-impl/lang-exp/anonmapping-clib/otel_process_ctx.c
...
otel_process_ctx_result otel_process_ctx_publish(otel_process_ctx_data data) {
  volatile otel_process_ctx_state *state = static_cast<volatile otel_process_ctx_state *>(calloc(1, sizeof(otel_process_ctx_state)));
  if (!state) {
  ...
  void* mapped = mmap(nullptr, sizeof(otel_process_ctx_mapping), PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  otel_process_ctx_mapping *mapping = static_cast<otel_process_ctx_mapping*>(mapped);
  ...
  *state->mapping = (otel_process_ctx_mapping) {
  ...
  // Optional step: Change permissions on the mapping to only execute permission
  // We've observed it's rare for anonymous mappings to have only execute permission by itself (e.g. without read), so this is left as a hint for
  // when running on older kernels and the naming below isn't available. For modern kernels, doing this is harmless so we do it
  // unconditionally.
  if (mprotect(state->mapping, sizeof(otel_process_ctx_mapping), PROT_EXEC) == -1) {
    otel_process_ctx_state_drop(state);
  ...
  if (prctl(PR_SET_VMA, PR_SET_VMA_ANON_NAME, state->mapping, sizeof(otel_process_ctx_mapping), "OTEL_CTX") == -1) {
    // Naming an anonymous mapping a Linux 5.17+ feature. On earlier versions, or not Linux, this method call can fail. Thus it's OK
...
static otel_process_ctx_result otel_process_ctx_encode_payload(char **out, uint32_t *out_size, otel_process_ctx_data data) {
  ...
  for (size_t i = 0; i < num_pairs; i++) {
    ...
    if (pairs[i][1] == nullptr) {
      return (otel_process_ctx_result) {.success = false, .error_message = "Value in otel_process_ctx_data is nullptr (" __FILE__ ":" ADD_QUOTES(__LINE__) ")"};
    }
  ...
  char *encoded = static_cast<char*>(calloc(total_size, 1));
  if (!encoded) {
  ...
  *out_size = (uint32_t) total_size;
  ...
  static void *parse_mapping_start(char *line) {
    ...
    if (start == 0 || start == ULLONG_MAX) return nullptr;
  ...
  static bool otel_process_ctx_decode_payload(char *payload, otel_process_ctx_data *data_out) {
    ...
    data_out->service_name = nullptr;
    data_out->service_instance_id = nullptr;
    data_out->deployment_environment_name = nullptr;
    ...
    for (int i = 0; i < count; i++) {
      ...
      // Read value
      char *value = static_cast<char*>(malloc(value_len + 1));
    ...
    return data_out->service_name != nullptr &&
           data_out->service_instance_id != nullptr &&
           data_out->deployment_environment_name != nullptr;
  }
  ...
  otel_process_ctx_read_result otel_process_ctx_read(void) {
    ...
    if (!mapping) {
      return (otel_process_ctx_read_result) {
        .success = false,
        .error_message = "No OTEL_CTX mapping found"
      };
    }

    // Temporarily change permissions on the mapping to read-only
    if (mprotect(mapping, sizeof(otel_process_ctx_mapping), PROT_READ) == -1) {
      return (otel_process_ctx_read_result) {
        .success = false,
        .error_message = "Failed to change mapping permissions to read-only"
      };
    }

    otel_process_ctx_read_result result = {.success = true};
    ...
    if (strncmp(mapping->otel_process_ctx_signature, "OTEL_CTX", sizeof(mapping->otel_process_ctx_signature)) != 0 || mapping->otel_process_ctx_version != 1) {
      result = (otel_process_ctx_read_result) {
        .success = false,
        .error_message = "Invalid OTEL_CTX signature or version"
      };
    }
    char *payload = mapping->otel_process_payload;

    // Restore permissions on the mapping
    if (mprotect(mapping, sizeof(otel_process_ctx_mapping), PROT_EXEC) == -1) {
      return (otel_process_ctx_read_result) {
        .success = false,
        .error_message = "Failed to restore mapping permissions"
      };
    }
    ...
    if (!otel_process_ctx_decode_payload(payload, &data)) {
      return (otel_process_ctx_read_result) {
        .success = false,
        .error_message = "Failed to decode payload"
      };
    }
