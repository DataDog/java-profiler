...
static inline ssize_t runStreamSocketHook(int fd, Fn fn, u8 op,
  ...
  bool sampler_active = NativeSocketSampler::active();
...
int NativeSocketInterposer::connect_hook(int fd, const struct sockaddr* addr,
                                         socklen_t addrlen) {
  connect_fn fn = _orig_connect.load(std::memory_order_acquire);
  auto call = [&](connect_fn fn) { return fn(fd, addr, addrlen); };
  if (fn == nullptr) {
    errno = ENOSYS;
    return -1;
  }
  ...
  return runNativeIoHook<int>(eligible, NativeBlockKind::CONNECT, fd, fn, call);
}

int NativeSocketInterposer::accept_hook(int fd, struct sockaddr* addr,
                                        socklen_t* addrlen) {
  accept_fn fn = _orig_accept.load(std::memory_order_acquire);
  auto call = [&](accept_fn fn) { return fn(fd, addr, addrlen); };
  if (fn == nullptr) {
    errno = ENOSYS;
    return -1;
  }
  ...
  return runNativeIoHook<int>(eligible, NativeBlockKind::ACCEPT, fd, fn, call);
}

int NativeSocketInterposer::accept4_hook(int fd, struct sockaddr* addr,
                                         socklen_t* addrlen, int flags) {
  accept4_fn fn = _orig_accept4.load(std::memory_order_acquire);
  auto call = [&](accept4_fn fn) { return fn(fd, addr, addrlen, flags); };
  if (fn == nullptr) {
    errno = ENOSYS;
    return -1;
  }
