...
void Lookup::fillJavaMethodInfo(MethodInfo* mi, jmethodID method, bool first_time) {
    ...
    if ((phase & (JVMTI_PHASE_START | JVMTI_PHASE_LIVE)) != 0) {
        ...
        if (VMMethod::check_jmethodID(method) && jvmti->GetMethodDeclaringClass(method, &method_class) == 0 &&
            ...
            } else if (strncmp(method_name, "main", 5) == 0 && strncmp(method_sig, "(Ljava/lang/String;)V", 21)) {
...
RecordingBuffer* Recording::buffer(int lock_index) {
...
void Recording::writeStringSetting(Buffer* buf, int category, const char* key, const char* value) {
...
void Recording::writeBoolSetting(Buffer* buf, int category, const char* key, bool value) {
    writeStringSetting(buf, category, key, value ? "true" : "false");
}

void Recording::writeIntSetting(Buffer* buf, int category, const char* key, long long value) {
    char str[32];
    snprintf(str, sizeof(str), "%lld", value);
    writeStringSetting(buf, category, key, str);
}

void Recording::writeListSetting(Buffer* buf, int category, const char* key, const char* base, int offset) {
    while (offset != 0) {
...
void Recording::writeDatadogSetting(Buffer* buf, int length, const char* name, const char* value, const char* unit) {
    flushIfNeeded(buf, RECORDING_BUFFER_LIMIT - length);
...
void Recording::writeDatadogProfilerConfig(Buffer* buf,
                                long cpuInterval,
                                long wallInterval,
                                long allocInterval,
...
void Recording::writeConstantPoolSection(Buffer* buf, JfrType type, std::map<u32, const char*>& constants) {
    flushIfNeeded(buf);
    buf->putVar64(type);
...
void Recording::writeConstantPoolSection(Buffer* buf, JfrType type, Dictionary* dictionary) {
...
void Recording::recordExecutionSample(Buffer* buf, int tid, u32 call_trace_id, ExecutionEvent* event) {
    int start = buf->skip(1);
...
void Recording::recordMethodSample(Buffer* buf, int tid, u32 call_trace_id, ExecutionEvent* event) {
    int start = buf->skip(1);
...
void Recording::recordHeapLiveObject(Buffer* buf, int tid, u32 call_trace_id, ObjectLivenessEvent* event) {
    int start = buf->skip(1);
...
void Recording::recordMonitorBlocked(Buffer* buf, int tid, u32 call_trace_id, LockEvent* event) {
    int start = buf->skip(1);
...
void Recording::recordThreadPark(Buffer* buf, int tid, u32 call_trace_id, LockEvent* event) {
    int start = buf->skip(1);
...
void Recording::recordCpuLoad(Buffer* buf, float proc_user, float proc_system, float machine_total) {
    int start = buf->skip(1);
