...
public:
  ...
  // tracking probability over the configured _subsample ratio (default 10% -
  ...
  // Frees this thread's subsampling RNG state (track()'s rng/skipped
  ...
  // reject for unboosted tids - xorshift::threshold(0) is 0, which no draw
  // compares <, so a ratio of 0 can never admit), and returns this thread's
  // rng ThreadLocal to the unseeded sentinel so the next admitForTracking()
  // draw comes from a freshly-seeded stream.
