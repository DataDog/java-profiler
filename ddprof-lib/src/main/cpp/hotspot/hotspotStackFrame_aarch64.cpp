...
NOSANALIGSANITIZE bool HotspotStackFrame::unwindPrologue(VMNMethod* nm, uintptr_t& pc, uintptr_t& sp, uintptr_t& fp) {
    // C1/C2 methods:
    //   {stack_bang}
    //   sub  sp, sp, #0x40
    //   stp  x29, x30, [sp, #48]
...
NOSANALIGSANITIZE bool HotspotStackFrame::unwindEpilogue(VMNMethod* nm, uintptr_t& pc, uintptr_t& sp, uintptr_t& fp) {
    //  ldp  x29, x30, [sp, #32]
    //  add  sp, sp, #0x30
    //  {poll_return}
    //  ret
