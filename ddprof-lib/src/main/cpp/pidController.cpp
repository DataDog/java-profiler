...
double PidController::compute(u64 input, double time_delta_coefficient) {
  ...
  // the values are linearly scaled using that coefficient to reinterpret the
  // given value within the expected sampling window
  double absolute_error =
      static_cast<double>(_target) - input * time_delta_coefficient;
  ...
  double signal = _proportional_gain * absolute_error +
                  _integral_gain * _integral_value +
                  _derivative_gain * derivative;
