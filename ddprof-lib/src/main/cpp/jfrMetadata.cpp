...
void JfrMetadata::initialize(const std::vector<std::string>& contextAttributes) {
    ...
    _root
        << (element("metadata")
            ...
            << (type("datadog.types.ExecutionMode", T_EXECUTION_MODE, "Exeuction Mode", true)
                << field("name", T_STRING, "Name"))
            ...
            << (type("datadog.CyclicBarrierTime", T_CYCLIC_BARRIER_TIME, "Cyclic Barrier Time")
                << category("Datadog")
                << field("startTime", T_LONG, "Start Time", F_TIME_TICKS)
                << field("duration", T_LONG, "Duration", F_DURATION_TICKS)
                << field("eventThread", T_THREAD, "Event Thread", F_CPOOL)
                << field("task", T_CLASS, "Task", F_CPOOL)
                << field("barrier", T_INT, "Barrier")
                << field("generation", T_INT, "Generation")
                << field("spanId", T_LONG, "Span ID")
                << field("localRootSpanId", T_LONG, "Local Root Span ID")
                || contextAttributes)
            ...
            << (type("jdk.ActiveRecording", T_ACTIVE_RECORDING, "java-profiler Recording")
                ...
                << field("flushInterval", T_LONG, "Flush Interval", F_DURATION_MILLIS, VM::java_version() >= 14)
            ...
            << (type("datadog.ProfilerSetting", T_DATADOG_SETTING, "Profiler Configuration Setting")
                << category("Datadog")
                << field("startTime", T_LONG, "Start Time", F_TIME_TICKS)
                << field("duration", T_LONG, "Duration", F_DURATION_TICKS)
                << field("eventThread", T_THREAD, "Event Thread", F_CPOOL)
            ...
            << (type("datadog.DatadogProfilerConfig", T_DATADOG_PROFILER_CONFIG, "Datadog Profiler Configuration")
                << category("Datadog")
                << field("startTime", T_LONG, "Start Time", F_TIME_TICKS)
                << field("duration", T_LONG, "Duration", F_DURATION_TICKS)
                << field("eventThread", T_THREAD, "Event Thread", F_CPOOL)
                << field("cpuInterval", T_LONG, "CPU Sampling Interval", F_DURATION_MILLIS)
                << field("wallInterval", T_LONG, "Wall Sampling Interval", F_DURATION_MILLIS)
                << field("allocInterval", T_LONG, "Allocation Sampling Interval", F_BYTES)
