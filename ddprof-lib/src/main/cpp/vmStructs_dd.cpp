...
int ddprof::VMStructs::_flag_type_offset = -1;
int ddprof::VMStructs::_osthread_state_offset = -1;

CodeCache ddprof::VMStructs::_unsafe_to_walk("unwalkable code");

ddprof::VMStructs::HeapUsageFunc ddprof::VMStructs::_heap_usage_func = NULL;
ddprof::VMStructs::MemoryUsageFunc ddprof::VMStructs::_memory_usage_func = NULL;
ddprof::VMStructs::GCHeapSummaryFunc ddprof::VMStructs::_gc_heap_summary_func = NULL;
ddprof::VMStructs::IsValidMethodFunc ddprof::VMStructs::_is_valid_method_func = NULL;


// Run at agent load time
void ddprof::VMStructs::init(CodeCache* libjvm) {
...
void ddprof::VMStructs::initOffsets() {
...
void ddprof::VMStructs::initUnsafeFunctions() {
  // see
...
const void *ddprof::VMStructs::findHeapUsageFunc() {
  if (VM::hotspot_version() < 17) {
    // For JDK 11 it is really unreliable to find the memory_usage function -
    // just disable it
    return nullptr;
  } else {
    ddprof::JVMFlag* flag = ddprof::JVMFlag::find("UseG1GC", {ddprof::JVMFlag::Type::Bool});
    ...
    flag = ddprof::JVMFlag::find("UseShenandoahGC", {ddprof::JVMFlag::Type::Bool});
    if (flag != NULL && flag->get()) {
      return _libjvm->findSymbol("_ZN14ShenandoahHeap12memory_usageEv");
    }
    flag = ddprof::JVMFlag::find("UseZGC", {ddprof::JVMFlag::Type::Bool});
...
void ddprof::VMStructs::initJvmFunctions() {
...
bool ddprof::HeapUsage::is_jmx_attempted = false;
bool ddprof::HeapUsage::is_jmx_supported = false; // default to not-supported

void ddprof::HeapUsage::initJMXUsage(JNIEnv *env) {
  if (is_jmx_attempted) {
 ...
 */
bool ddprof::VMMethod::check_jmethodID(jmethodID id) {
  if (id == NULL) {
    return false;
  }
  if (VM::isOpenJ9()) {
...
bool ddprof::VMMethod::check_jmethodID_hotspot(jmethodID id) {
  ...
  ddprof::VMStructs::IsValidMethodFunc func = ddprof::VMStructs::is_valid_method_func();
  if (func != NULL) {
  ...
  if (ddprof::VMStructs::method_constmethod_offset() >= 0) {
    const_method = (const char *)SafeAccess::load(
        (void **)(method_ptr + ddprof::VMStructs::method_constmethod_offset()));
    if (const_method == NULL) {
      return false;
    }
  }
  if (ddprof::VMStructs::constmethod_constants_offset() >= 0) {
    cpool = (const char *)SafeAccess::load(
        (void **)(const_method + ddprof::VMStructs::constmethod_constants_offset()));
    if (cpool == NULL) {
      return false;
    }
  }
  if (ddprof::VMStructs::pool_holder_offset() >= 0) {
    cpool_holder =
        (const char *)SafeAccess::load((void **)(cpool + ddprof::VMStructs::pool_holder_offset()));
    if (cpool_holder == NULL) {
      return false;
    }
  }
  if (ddprof::VMStructs::class_loader_data_offset() >= 0) {
    cl_data = (const char *)SafeAccess::load(
        (void **)(cpool_holder + ddprof::VMStructs::class_loader_data_offset()));
    if (cl_data == NULL) {
      return false;
    }
  }
...
bool ddprof::VMMethod::check_jmethodID_J9(jmethodID id) {
  // the J9 jmethodid check is not working properly, so we just check for NULL
  return id != NULL && *((void **)id) != NULL;
}

ddprof::JVMFlag *ddprof::JVMFlag::find(const char *name) {
  if (_flags_addr != NULL && _flag_size > 0) {
    for (int i = 0; i < _flag_count; i++) {
      ddprof::JVMFlag *f = (ddprof::JVMFlag *)(_flags_addr + i * _flag_size);
...
ddprof::JVMFlag *ddprof::JVMFlag::find(const char *name, std::initializer_list<ddprof::JVMFlag::Type> types) {
  int mask = 0;
  for (int type : types) {
    mask |= 0x1 << type;
  }
...
int ddprof::JVMFlag::type() {
  if (VM::hotspot_version() < 16) { // in JDK 16 the JVM flag implementation has changed
    char* type_name = *(char **)at(_flag_type_offset);
    if (type_name == NULL) {
      return ddprof::JVMFlag::Type::Unknown;
    }
    if (strcmp(type_name, "bool") == 0) {
      return ddprof::JVMFlag::Type::Bool;
    } else if (strcmp(type_name, "int") == 0) {
      return ddprof::JVMFlag::Type::Int;
    } else if (strcmp(type_name, "uint") == 0) {
      return ddprof::JVMFlag::Type::Uint;
    } else if (strcmp(type_name, "intx") == 0) {
      return ddprof::JVMFlag::Type::Intx;
    } else if (strcmp(type_name, "uintx") == 0) {
      return ddprof::JVMFlag::Type::Uintx;
    } else if (strcmp(type_name, "uint64_t") == 0) {
      return ddprof::JVMFlag::Type::Uint64_t;
    } else if (strcmp(type_name, "size_t") == 0) {
      return ddprof::JVMFlag::Type::Size_t;
    } else if (strcmp(type_name, "double") == 0) {
        return ddprof::JVMFlag::Type::Double;
    } else if (strcmp(type_name, "ccstr") == 0) {
      return ddprof::JVMFlag::Type::String;
    } else if (strcmp(type_name, "ccstrlist") == 0) {
      return ddprof::JVMFlag::Type::Stringlist;
    } else {
      return ddprof::JVMFlag::Type::Unknown;
    }
  } else {
...
ddprof::JVMFlag *ddprof::JVMFlag::find(const char *name, int type_mask) {
  if (_flags_addr != NULL && _flag_size > 0) {
    for (int i = 0; i < _flag_count; i++) {
      ddprof::JVMFlag *f = (ddprof::JVMFlag *)(_flags_addr + i * _flag_size);
...
bool ddprof::VMStructs::isSafeToWalk(uintptr_t pc) {
  return !(_unsafe_to_walk.contains((const void *)pc) &&
           _unsafe_to_walk.findFrameDesc((const void *)pc));
}

void ddprof::VMStructs::NativeMethodBind(jvmtiEnv *jvmti, JNIEnv *jni, jthread thread,
...
void ddprof::VMStructs::checkNativeBinding(jvmtiEnv *jvmti, JNIEnv *jni,
                                   jmethodID method, void *address) {
  char *method_name;
  char *method_sig;
  int error = 0;
