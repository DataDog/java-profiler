...
bool JVM::is_readable_pointer(void *ptr) {
  if (!_is_readable_pointer_resolved) {
    ...
    _is_readable_pointer =
        (is_readable_pointer_fn)VMStructs::libjvm()->findSymbol(sym_name);
    if (_is_readable_pointer == NULL) {
      Log::error("Can not resolve symbol %s in JVM lib\n", sym_name);
    }
  }
  // if we have no access to the symbol we can not really attempt the pointer
  // sanitation
