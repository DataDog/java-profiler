...
int Profiler::getJavaTraceAsync(void* ucontext, ASGCT_CallFrame* frames, int max_depth, StackContext* java_ctx, bool *truncated) {
    ...
    if (ucontext != NULL) {
        ...
        if (!VMStructs::isSafeToWalk(saved_pc)) {
            ...
            CodeBlob* codeBlob = VMStructs::libjvm()->findBlobByAddress((const void*) saved_pc);
            if (codeBlob) {
...
void Profiler::recordCyclicBarrierTime(int tid, CyclicBarrierTimeEvent* event) {
    u32 lock_index = getLockIndex(tid);
    if (!_locks[lock_index].tryLock() &&
        !_locks[lock_index = (lock_index + 1) % CONCURRENCY_LEVEL].tryLock() &&
        !_locks[lock_index = (lock_index + 2) % CONCURRENCY_LEVEL].tryLock()) {
    ...
    _jfr.recordCyclicBarrierTime(lock_index, tid, event);
    _locks[lock_index].unlock();
}
