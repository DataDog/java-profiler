...
TEST_F(RemoteSymbolicationTest, RemoteFrameInfoConstruction) {
    // Test build-id as hex string pointer
    const char* test_build_id_hex = "deadbeefcafebabe";
    uintptr_t test_offset = 0x1234;
    short test_lib_index = 5;

    // Initialize RemoteFrameInfo with hex string pointer
    RemoteFrameInfo rfi;
    rfi.build_id_hex = test_build_id_hex;
    rfi.pc_offset = test_offset;
    rfi.lib_index = test_lib_index;

    // Verify fields
    EXPECT_STREQ(rfi.build_id_hex, "deadbeefcafebabe");
