...
# publish-gh-pages.sh - Publish reliability test reports to GitHub Pages
#
# Usage: publish-gh-pages.sh [artifacts-dir]
#
# Updates reliability history and regenerates GitHub Pages site.
# Reports are available at: https://datadog.github.io/async-profiler-build/reliability/
#
...
# HISTORY TRACKING
# ============================================
log_info "Updating reliability test history..."

# Generate run JSON for this pipeline
if "${SCRIPT_DIR}/generate-run-json.sh" "${ARTIFACTS_DIR}" > "${RUN_JSON_FILE}" 2>/dev/null; then
  log_info "Generated run JSON"

  # Update history (prepend new run, keep last MAX_HISTORY)
  if "${SCRIPT_DIR}/../common/update-history.sh" reliability "${RUN_JSON_FILE}" "." 2>/dev/null; then
    log_info "Updated reliability history"
  else
...
log_info "Generating reliability index..."
if "${SCRIPT_DIR}/../common/generate-index.sh" reliability "." 2>/dev/null; then
  log_info "Generated reliability/index.md"
else
  log_warn "Failed to generate reliability index"
fi
...
if git diff --staged --quiet; then
  ...
  git commit -m "Update reliability reports - ${TIMESTAMP}"

  log_info "Pushing to gh-pages..."
  git push origin gh-pages --force

  log_info "Reports published successfully!"
  log_info "View at: ${PAGES_URL}/reliability/"
fi

echo ""
echo "PAGES_URL=${PAGES_URL}/reliability/"
