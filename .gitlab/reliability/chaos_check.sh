...
case $CONFIG in
  profiler)
    ...
    ANTAGONISTS="thread-churn,alloc-storm,vthread-churn,classloader-churn,dump-storm"
    ;;
  profiler+tracer)
    echo "Running with profiler and tracer"
    ENABLEMENT="-Ddd.profiling.enabled=true -Ddd.trace.enabled=true"
    ANTAGONISTS="thread-churn,alloc-storm,vthread-churn,classloader-churn,trace-context,dump-storm"
    ;;
  *)
