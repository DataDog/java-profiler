...
with open(log_path, 'r', encoding='utf-8', errors='replace') as f:
    for line in f:
        ...
        if re.match(r'^> Task .* SKIPPED', line):
            task_skipped = re.match(r'^> Task ([^\s]+) SKIPPED', line)
            if task_skipped:
                task_name = task_skipped.group(1)
                tasks[task_name] = {"status": "SKIPPED", "reason": ""}

        # Skipping reason
        if re.search(r"Skipping task '([^']+)' as (.+)", line):
            skip_match = re.search(r"Skipping task '([^']+)' as (.+)", line)
            if skip_match:
                skip_task = skip_match.group(1)
                skip_reason = skip_match.group(2)
                if skip_task in tasks:
                    tasks[skip_task]["reason"] = skip_reason
                skipped_reason[skip_task] = skip_reason
...
for task_name, task_info in tasks.items():
    if task_info.get("status") == "SKIPPED":
        result["skippedTasks"].append({
            "task": task_name,
            "reason": task_info.get("reason", skipped_reason.get(task_name, "Unknown"))
        })
...
if result["skippedTasks"]:
    for skipped in result["skippedTasks"]:
        if ":test" in skipped["task"]:
            result["actions"].append(f"Test task {skipped['task']} was skipped: {skipped['reason']}")
if result["warnings"]:
...
md_lines.append(f"**Log File:** build/logs/20260126-094600-_ddprof-test_test.log")
md_lines.append("")

# Task summary
if result["skippedTasks"]:
    md_lines.append("## Skipped Tasks")
    md_lines.append("")
    for skipped in result["skippedTasks"]:
        md_lines.append(f"- **{skipped['task']}**")
        md_lines.append(f"  - Reason: {skipped['reason']}")
    md_lines.append("")
...
if result["tests"]["total"] > 0:
    ...
    if result["skippedTasks"]:
        for skipped in result["skippedTasks"]:
            if ":test" in skipped["task"]:
                md_lines.append(f"Note: {skipped['task']} was skipped - {skipped['reason']}")
    md_lines.append("")
...
print(f"Analysis complete. Generated {len(md_lines)} lines in markdown summary.")
print(f"Build status: {result['status']} in {result['totalTime']}")
print(f"Skipped tasks: {len(result['skippedTasks'])}")
